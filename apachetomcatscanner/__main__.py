...
from apachetomcatscanner.utils.scan import (monitor_thread, scan_worker,
                                            scan_worker_url)
...
def load_targets(options, config):
    ...
    if (
        ...
        and options.servers_only is False
    ):
        if options.debug:
            print(
    ...
    if (
        ...
        and options.auth_user is not None
        and (options.auth_password is not None or options.auth_hashes is not None)
        and options.servers_only is True
    ):
        if options.debug:
            print(
    ...
    if options.targets_urls_file is not None:
        if os.path.exists(options.targets_urls_file):
            if options.debug:
                print(
                    "[debug] Loading target URLs line by line from targets urls file '%s'"
                    % options.targets_urls_file
                )
...
def parseArgs():
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Debug mode, for huge verbosity. (default: False)",
    )
    parser.add_argument(
        "-C",
        "--list-cves",
        default=False,
        action="store_true",
        help="List CVE ids affecting each version found. (default: False)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="If querying ActiveDirectory, only get servers and not all computer objects. (default: False)",
    )
    parser.add_argument(
        "--no-colors",
        default=False,
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Scan only with HTTP scheme. (default: False, scanning with both HTTP and HTTPs)",
    )
    parser.add_argument(
        "--only-https",
        default=False,
        action="store_true",
        help="Scan only with HTTPs scheme. (default: False, scanning with both HTTP and HTTPs)",
    )
    ...
    group_export.add_argument(
        ...
        default=None,
        required=False,
        help="Output XLSX file to store the results in.",
    )
    group_export.add_argument(
        "--export-json",
        dest="export_json",
        ...
        default=None,
        required=False,
        help="Output JSON file to store the results in.",
    )
    group_export.add_argument(
        "--export-sqlite",
        dest="export_sqlite",
        ...
        default=None,
        required=False,
        help="Output SQLITE3 file to store the results in.",
    )

    group_configuration = parser.add_argument_group("Advanced configuration")
    group_configuration.add_argument(
        "-PI", "--proxy-ip", default=None, type=str, help="Proxy IP."
    )
    group_configuration.add_argument(
        "-PP", "--proxy-port", default=None, type=int, help="Proxy port"
    )
    group_configuration.add_argument(
        "-rt",
        "--request-timeout",
        default=5,
        type=int,
        help="Set the timeout of HTTP requests.",
    )
    group_configuration.add_argument(
        "-H",
        "--http-header",
        dest="request_http_header",
        default=[],
        type=str,
        action="append",
        help="Custom HTTP headers to add to requests.",
    )
    group_configuration.add_argument(
        "--tomcat-username",
        default=None,
        help="Single tomcat username to test for login.",
    )
    group_configuration.add_argument(
        "--tomcat-usernames-file",
        default=None,
        help="File containing a list of tomcat usernames to test for login",
    )
    group_configuration.add_argument(
        "--tomcat-password",
        default=None,
        help="Single tomcat password to test for login.",
    )
    group_configuration.add_argument(
        "--tomcat-passwords-file",
        default=None,
        help="File containing a list of tomcat passwords to test for login",
    )
    ...
    group_targets_source.add_argument(
        ...
        type=str,
        action="append",
        help="Target URL to the tomcat manager.",
    )
    group_targets_source.add_argument(
        "-tU",
        "--targets-urls-file",
        default=None,
        type=str,
        help="Path to file containing a line by line list of target URLs.",
    )
    group_targets_source.add_argument(
        "-tp",
        "--target-ports",
        default="80,443,8080,8081,8180,9080,9081,10080",
        type=str,
        help="Target ports to scan top search for Apache Tomcat servers.",
    )
    group_targets_source.add_argument(
        "-ad",
        "--auth-domain",
...
def main():
    ...
    config.set_request_available_schemes(
        only_http=options.only_http, only_https=options.only_https
    )
    ...
    number_of_tested_credentials = config.load_credentials_from_options(
        options.tomcat_username,
        options.tomcat_password,
        options.tomcat_usernames_file,
        options.tomcat_passwords_file,
    )
    if config.verbose_mode:
        print(
            "[verbose] %s credentials will be tested per target"
    ...
    if total_targets != 0:
        if options.proxy_ip is not None and options.proxy_port is not None:
            if len(targets_others) != 0 and len(targets_urls) != 0:
                print(
                    "[+] Targeting %d ports on %d hosts, and %d urls, through proxy %s:%d."
                    % (
                        len(ports),
                        len(targets_others),
                        len(targets_urls),
                ...
                print(
                    "[+] Targeting %d urls, through proxy %s:%d."
                    % (len(targets_urls), options.proxy_ip, options.proxy_port)
                )
            elif len(targets_others) != 0 and len(targets_urls) == 0:
                print(
                    "[+] Targeting %d ports on %d hosts, through proxy %s:%d."
                    % (
                        len(ports),
                        len(targets_others),
                        options.proxy_ip,
            ...
            if len(targets_others) != 0 and len(targets_urls) != 0:
                print(
                    "[+] Targeting %d ports on %d hosts, and %d urls."
                ...
                print(
                    "[+] Targeting %d ports on %d hosts."
                    % (len(ports), len(targets_others))
                )
        ...
        if len(targets) != 0 and options.threads != 0:
            ...
            with ThreadPoolExecutor(
                max_workers=min(options.threads, 1 + monitor_data["total"])
            ) as tp:
                tp.submit(monitor_thread, reporter, config, monitor_data)
                for target_type, target in targets:
                    if target_type == "url":
                        tp.submit(
                            scan_worker_url, target, reporter, config, monitor_data
                        )
                    else:
                        for port in ports:
                            tp.submit(
